...
export class MovingBehavior {
    ...
    attachMovingBehavior(ownerCharacter: Game_CharacterBase, targetCharacter: Game_CharacterBase) {
    ...
    detach() {
        this.ownerCharacter()._movingBehavior = undefined;
 ...
 * 押して移動
 */
export class MovingBehavior_PushMoving extends MovingBehavior {
    ...
    static tryStartPushObjectAndMove(character: Game_CharacterBase, d: number): boolean {
        ...
        if (this.tryMoveAsPushableObject(target, d)) {
            ...
            if (character.isMovementSucceeded(character.x, character.y)) {
                behavior.attachMovingBehavior(character, target);
                return true;
            }
        }
